...
# ─────────────────────────────────────────────────────────────────────────────
# NOTE: Kubernetes resources are now managed by bootstrap-aks.sh, NOT Terraform.
# This function is kept for cleanup of ArgoCD Applications and finalizers that may
# still exist in the cluster. However, Terraform will no longer try to delete
# Kubernetes namespaces/secrets, so destroy should complete quickly.
#
cleanup_kubernetes_resources() {
  section "Cleaning up ArgoCD Applications (optional)"

  local kubeconfig_app="${SCRIPT_DIR}/kubeconfigs/merged-admin.yaml"
  if [ ! -f "${kubeconfig_app}" ]; then
    info "Admin kubeconfig not found — skipping Kubernetes cleanup"
    return 0
  fi
  ...
  for ctx in shelfware-app shelfware-loadtest; do
    if ! kubectl cluster-info --context "${ctx}" >/dev/null 2>&1; then
      info "${ctx} unreachable — skipping (cluster may already be gone)"
      continue
    fi

    info "=== Cleaning up ${ctx} ==="

    # 1. Scale ArgoCD to zero so it stops reconciling
    kubectl scale deployment -n argocd --all --replicas=0 --context "${ctx}" >/dev/null 2>&1 || true
    info "  ArgoCD scaled to 0 ✓"

    # 2. Remove finalizers from ArgoCD Applications to speed cleanup
...
" 2>/dev/null || true
    ...
    info "  ArgoCD Applications cleaned up ✓"
